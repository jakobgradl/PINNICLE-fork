...
### SSA_weak for PINN:
class SSAweakEquationParameter(EquationParameter, Constants):
    """ default parameters for SSA
    """
    _EQUATION_TYPE = 'SSAweak' 
    def __init__(self, param_dict={}):
...
class SSAweak(EquationBase): #{{{
    """ SSA on 2D problem with uniform B
    """
    _EQUATION_TYPE = 'SSAweak' 
    def __init__(self, parameters=SSAweakEquationParameter()):
        super().__init__(parameters)
    def _pde(self, nn_input_var, nn_output_var): #{{{
