...
def test_SSA_Taub_pde_function():
    hp_local = dict(hp)
    hp_local["equations"] = {"SSA Taub":{}}
    experiment = pinn.PINN(params=hp_local)
    experiment.compile()
    y = experiment.model.predict(experiment.model_data.X['u'], operator=experiment.physics.operator("SSA Taub"))
    assert len(y) == 2
    assert y[0].shape == (10,1)
    assert y[1].shape == (10,1)

def test_SSA_MC_Taub_pde_function():
    hp_local = dict(hp)
    hp_local["equations"] = {"SSA_MC Taub":{}}
    experiment = pinn.PINN(params=hp_local)
    experiment.compile()
    y = experiment.model.predict(experiment.model_data.X['u'], operator=experiment.physics.operator("SSA_MC Taub"))
    assert len(y) == 2
    assert y[0].shape == (10,1)
    assert y[1].shape == (10,1)
